...
function buildTopGeometry(
  ...
  const buildFoldedCollar = (
    ...
    const mkPoint = (jEnd: number, sx: number) => {
      ...
      // Button-down collar: the defining feature of an oxford is that each
      // collar point fastens to the chest. Without it an oxford and a casual
      // shirt render as the same garment. The tip is already computed above,
      // so the button is a small disc sitting just proud of it.
      if (features.collarButtons) {
        const br = 0.5 * SCALE;
        const SEGMENTS = 10;
        const bx = tip.x - sx * 0.5 * SCALE;
        const by = tip.y + 0.9 * SCALE;
        const bz = tip.z + 0.22 * SCALE;
        const hub = pushVertex(bx, by, bz, 0.5, 0.5);
        const rim: number[] = [];
